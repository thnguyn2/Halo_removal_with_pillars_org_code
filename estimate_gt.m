function [gk,tk,tk0] = estimate_gt(gamma,h,niter,lambda,beta_weight,tol,method,bg)
   ...
   % gamma = gamma/maxgamma;
    ...
    for iter=1:niter
        ...
        if (mod(iter,100)==1)
            figure(4);
            subplot(1,3,1);imagesc(angle(tk));colorbar;colormap gray;title(sprintf('Current estimation tk - iter #%d',iter));
            subplot(1,3,2);imagesc(angle(gk));colormap gray;colorbar;title(sprintf('Current estimation -gk iter #%d',iter));
            subplot(1,3,3);imagesc(angle(gamma));colormap gray;colorbar;title(sprintf('Original arg(gamma) iter #%d',iter));
